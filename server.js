...
const pool = mysql.createPool(client);
...
app.get(pageIndex.routePath, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            ...
            conn.query(pageIndex.portfolio(), function(portfolioerr, portfolio_result) {
                portfolioerr ? console.log("Index Portfolio Error")
                : (()=> {
                    conn.query(pageIndex.sourcecode(), function(sourceErr, sourceCode_result) {
                        sourceErr ? console.log("Index SourceCode Error")
                        : (()=>{
                            conn.query(pageIndex.itdev(), function(itdevErr, itdev_result) {
                                itdevErr ? console.log("Index ItDev Error")
                                : (()=> {
                                    conn.query(pageIndex.notice(), function(noticeErr, notice_result) {
                                        noticeErr ? console.log("Index Notice Error")
                                        : (()=> {
                                            conn.query(pageIndex.freeBoard(), function(freeBoardErr, freeBoard_result) {
...
passport.use(new localStrategy({
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            console.log("Login Pool Connection Error : " + err);
        } else {
            conn.query(Members_Query.Login(userID), function (loginErr, result) {
...
app.post(pageMembers.signup_RoutePath, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            console.log("Sign-Up Pool Connection Error : "+ err);
        } else {
            conn.query(Members_Query.isNew(userID), function(isNewerr, newtf) {
                isNewerr ? console.log(isNewerr)
                : (newtf[0].cnt != 0) ? (()=> {
                ...
                : (()=> {
                    ...
                    conn.query(Members_Query.SignUp(userID, userName, encryptedPassword, userEmail, 'USER'), function (pwderr) {
                        pwderr? console.log(`Insert Contents Error ${pwderr}`) 
...
app.post(pageMembers.findinfo_RoutePath, function (req, res) {
    ...
    if(userName) {
        pool.getConnection((err, conn)=> {
            if(err) {
                console.log("Find ID Info Pool Connection Error : "+ err);
            } else {
                conn.query(Members_Query.findID(userName, userEmail), function (findIDerr, searchedID) {
                    findIDerr? console.log(`Insert Contents Error ${findIDerr}`) 
                    : (()=> {
                        res.writeHead(200, {'Content-Type': 'text/html; charset=utf-8'});
        ...
        pool.getConnection((err, conn)=> {
            if(err) {
                console.log("Find PWD Info Pool Connection Error : "+err);
            } else {
                conn.query(Members_Query.findPWD(userID, p_userEmail), function (findPWDerr, searchedInfo) {
                    if(findPWDerr) {console.log(findPWDerr);}
                    else {
...
app.get(pageMembers.modifyInfo_RoutePath, function(req, res) {
    fs.readFile(pageMembers.modifyInfo_HTML, pageEncoding, function(htmlErr, data) {
        htmlErr ? console.log(`Read HTML Error ${htmlErr}`)
        : (()=> {
            pool.getConnection((err, conn)=> {
                if(err) {
                    console.log("Modify Memebers Info_Get Pool Connection Error : "+err);
                } else {
                    conn.query(Members_Query.Modify(req.params.id), function(error, result) {
...
app.post(pageMembers.modifyInfo_RoutePath, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            console.log("Modify Members Info_Post Pool Connection Error : "+err);
        } else {
            conn.query(Members_Query.isNew(userID), function(isNewErr, newtf) {
                isNewErr ? console.log(isNewErr)
                : (()=> {
                    const encryptedPassword = bcrypt.hashSync(userPWD, 10);
                    conn.query(Members_Query.changeInfo(userID, userName, encryptedPassword, userEmail), function (changeInfoErr) {
                        changeInfoErr? console.log(`Update Information Error ${changeInfoErr}`) 
                        : res.redirect(pageMembers.redirect_RoutePath());
                    });
                })()
            });
...
app.get(pageNotice.list_RoutePath, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            ...
            conn.query(NoticeBoard_Query.Count(), function (cnt_error, settings) {
                ...
                conn.query(NoticeBoard_Query.List(pagedata["start"], pagedata["items"]), function (list_err, list_result) {
...
app.post(pageNotice.write_RoutePath, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            console.log("Write Notice Pool Connection Error : "+err);
        } else {
            conn.query(NoticeBoard_Query.Write(content_kind, content_tit, content_txt, cont_date, 0), function (err) {
...
app.get(pageNotice.view_RoutePath, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            ...
            conn.query(NoticeBoard_Query.View(req.params.notice_no), function (view_data_err, result) {
                view_data_err ? console.log(`Getting Notice Content Data Error ${view_data_err}`) 
                : (()=> {
                    conn.query(NoticeBoard_Query.ViewCnt(req.params.notice_no), (view_cnt_err, updateResult)=>{
...
app.get(pageNotice.modify_RoutePath, chk_Login, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            ...
            conn.query(NoticeBoard_Query.View(req.params.notice_no), function(viewErr, result) {
...
app.post(pageNotice.modify_RoutePath, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            console.log("Modify Notice_Post Pool Connection Error : "+err);
        } else {
            conn.query(NoticeBoard_Query.Modify(content_kind, content_tit, content_txt, notice_no), function (err) {
                err? console.log(`Insert Contents Error ${err}`) 
...
app.get(pageNotice.delete_RoutePath, function(req, res) {
    pool.getConnection((err, conn)=> {
        if(err) {
            console.log("Delete Notice Pool Connection Error : "+err);
        } else {
            conn.query(NoticeBoard_Query.Delete(req.params.notice_no), function(delErr) {
                delErr ? console.log('Delete Error'+delErr)
                : (()=> {
                    res.writeHead(200, {'Content-Type': 'text/html; charset=utf-8'});
...
const pageFreeBoard = {
    ...
    delete_RoutePath : "/board/delete/free/:free_no",
...
const FreeBoard_Query = {
    ...
    Delete : function(free_no) {
        return `Delete From ${pageFreeBoard.TB_Name} Where ${pageFreeBoard.TB_PK} = ${free_no}`;
    }
}
...
app.get(pageFreeBoard.list_RoutePath, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            ...
            conn.query(FreeBoard_Query.Count(), function (cnt_error, settings) {
                cnt_error ? console.log(`Setting Data Error ${cnt_error}`) 
                : (()=> {
                    pagedata = {
                ...
                conn.query(FreeBoard_Query.List(pagedata["start"], pagedata["items"]), function (list_err, list_result) {
                    list_err ? console.log(`Getting Data Error ${list_err}`) 
                    : (()=> {
...
app.post(pageFreeBoard.write_RoutePath, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            console.log("Write FreeBoard Pool Connecntion Error : "+err);
        } else {
            conn.query(FreeBoard_Query.Write(content_user, content_tit, content_txt, cont_date, 0), function (err) {
                err? console.log(`Insert Contents Error ${err}`) 
...
app.get(pageFreeBoard.view_RoutePath, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            ...
            conn.query(FreeBoard_Query.View(req.params.free_no), function (view_data_err, result) {
                view_data_err ? console.log(`Getting FreeBoard Content Data Error ${view_data_err}`) 
                : (()=> {
                    conn.query(FreeBoard_Query.ViewCnt(req.params.free_no), (view_cnt_err, updateResult)=>{
                        if(view_cnt_err) console.log(`Increase View Count Error ${view_cnt_err}`);
                    }); 
...
app.get(pageFreeBoard.modify_RoutePath, chk_Login, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            ...
            conn.query(FreeBoard_Query.View(req.params.free_no), function(err, result) {
...
app.post(pageFreeBoard.modify_RoutePath, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            console.log("Modify FreeBoard_Post Pool Connection Error : "+err);
        } else {
            conn.query(FreeBoard_Query.Modify(content_tit, content_txt, free_no), function (err) {
                err? console.log(`Insert Contents Error ${err}`) 
...
app.get(pageFreeBoard.delete_RoutePath, function(req, res) {
    pool.getConnection((err, conn)=> {
        if(err) {
            console.log("Delete FreeBoard Pool Connection Error : "+err);
        } else {
            conn.query(FreeBoard_Query.Delete(req.params.free_no), function(delErr) {
                delErr ? console.log('Delete Error'+delErr)
                : (()=> {
                    res.writeHead(200, {'Content-Type': 'text/html; charset=utf-8'});
                    res.write("<script>alert('삭제 완료되었습니다.')</script>");
                    res.write(`<script>location.href = '${pageFreeBoard.redirect_RoutePath(10)}'</script>`);
                })();
            });
        } conn.release();
    });
...
const pagePortfolio = {
    ...
    delete_RoutePath : "/board/delete/portfolio/:portfolio_no",
...
const Portfolio_Query = {
    ...
    Delete : function(portfolio_no) {
        return `Delete From ${pagePortfolio.TB_Name} Where ${pagePortfolio.TB_PK} = ${portfolio_no}`;
    }
};
...
app.get(pagePortfolio.list_RoutePath, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            ...
            conn.query(Portfolio_Query.Count(), function (cnt_error, settings) {
                cnt_error ? console.log(`Setting Data Error ${cnt_error}`) 
                : (()=> {
                    pagedata = {
                ...
                conn.query(Portfolio_Query.List(pagedata["start"], pagedata["items"]), function (list_err, list_result) {
                    list_err ? console.log(`Getting Data Error ${list_err}`) 
                    : (()=> {
...
app.post(pagePortfolio.write_RoutePath, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            console.log("Write Portfolio Pool Connection Error : "+err);
        } else {
            conn.query(Portfolio_Query.Write(content_lang, content_tit, content_txt, cont_date, 0), function (err) {
                err? console.log(`Insert Contents Error ${err}`) 
...
app.get(pagePortfolio.view_RoutePath, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            ...
            conn.query(Portfolio_Query.View(req.params.portfolio_no), function (view_data_err, result) {
                view_data_err ? console.log(`Getting Portfolio Content Data Error ${view_data_err}`) 
                : (()=> {
                    conn.query(Portfolio_Query.ViewCnt(req.params.portfolio_no), (view_cnt_err, updateResult)=>{
                        if(view_cnt_err) console.log(`Increase View Count Error ${view_cnt_err}`);
                    }); 
...
app.get(pagePortfolio.modify_RoutePath, chk_Login, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            ...
            conn.query(Portfolio_Query.View(req.params.portfolio_no), function(err, result) {
...
app.post(pagePortfolio.modify_RoutePath, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            console.log("Modify Portfolio_Post Pool Connection Error : "+err);
        } else {
            conn.query(Portfolio_Query.Modify(content_lang, content_tit, content_txt, portfolio_no), function (inserterr) {
                inserterr? console.log(`Insert Contents Error ${inserterr}`) 
...
app.get(pagePortfolio.delete_RoutePath, function(req, res) {
    pool.getConnection((err, conn)=> {
        if(err) {
            console.log("Delete Portfolio Pool Connection Error : "+err);
        } else {
            conn.query(Portfolio_Query.Delete(req.params.portfolio_no), function(delErr) {
                delErr ? console.log('Delete Error'+delErr)
                : (()=> {
                    res.writeHead(200, {'Content-Type': 'text/html; charset=utf-8'});
                    res.write("<script>alert('삭제 완료되었습니다.')</script>");
                    res.write(`<script>location.href = '${pagePortfolio.redirect_RoutePath(10)}'</script>`);
                })();
            });
        } conn.release();
    });
...
const pageItDevDiary = {
    ...
    delete_RoutePath : "/board/delete/itdev/:itdev_no",
...
const ItDevDiary_Query = {
    ...
    Delete : function(itdev_no) {
        return `Delete From ${pageItDevDiary.TB_Name} Where ${pageItDevDiary.TB_PK} = ${itdev_no}`;
    }
};
...
app.get(pageItDevDiary.list_RoutePath, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            ...
            conn.query(ItDevDiary_Query.Count(), function (cnt_error, settings) {
                cnt_error ? console.log(`Setting Data Error ${cnt_error}`) 
                : (()=> {
                    pagedata = {
                ...
                conn.query(ItDevDiary_Query.List(pagedata["start"], pagedata["items"]), function (list_err, list_result) {
                    list_err ? console.log(`Getting Data Error ${list_err}`) 
                    : (()=> {
...
app.post(pageItDevDiary.write_RoutePath, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            console.log("Write ItDevDiary Pool Connecntion Error : "+err);
        } else {
            conn.query(ItDevDiary_Query.Write(content_lang, content_tit, content_txt, cont_date, 0), function (insertErr) {
                insertErr? console.log(`Insert Contents Error ${insertErr}`) 
...
app.get(pageItDevDiary.view_RoutePath, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            ...
            conn.query(ItDevDiary_Query.View(req.params.itdev_no), function (view_data_err, result) {
                view_data_err ? console.log(`Getting ItDevDiary Content Data Error ${view_data_err}`) 
                : (()=> {
                    conn.query(ItDevDiary_Query.ViewCnt(req.params.itdev_no), (view_cnt_err, updateResult)=>{
                        if(view_cnt_err) console.log(`Increase View Count Error ${view_cnt_err}`);
                    }); 
...
app.get(pageItDevDiary.modify_RoutePath, chk_Login, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            ...
            conn.query(ItDevDiary_Query.View(req.params.itdev_no), function(err, result) {
...
app.post(pageItDevDiary.modify_RoutePath, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            console.log("Modify ItDevDiary_Post Pool Connection Error : "+err);
        } else {
            conn.query(ItDevDiary_Query.Modify(content_lang, content_tit, content_txt, itdev_no), function (inserterr) {
                inserterr? console.log(`Insert Contents Error ${inserterr}`) 
...
app.get(pageItDevDiary.delete_RoutePath, function(req, res) {
    pool.getConnection((err, conn)=> {
        if(err) {
            console.log("Delete ItDevDiary Pool Connection Error : "+err);
        } else {
            conn.query(ItDevDiary_Query.Delete(req.params.itdev_no), function(delErr) {
                delErr ? console.log('Delete Error'+delErr)
                : (()=> {
                    res.writeHead(200, {'Content-Type': 'text/html; charset=utf-8'});
                    res.write("<script>alert('삭제 완료되었습니다.')</script>");
                    res.write(`<script>location.href = '${pageItDevDiary.redirect_RoutePath(10)}'</script>`);
                })();
            });
        } conn.release();
    });
...
const pageSourceCode = {
    ...
    delete_RoutePath : "/board/delete/sourcecode/:sCode_no",
...
const SourceCode_Query = {
    ...
    Delete : function(sCode_no) {
        return `Delete From ${pageSourceCode.TB_Name} Where ${pageSourceCode.TB_PK} = ${sCode_no}`;
    }
};
...
app.get(pageSourceCode.list_RoutePath, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            ...
            conn.query(SourceCode_Query.Count(), function (cnt_error, settings) {
                cnt_error ? console.log(`Setting Data Error ${cnt_error}`) 
                : (()=> {
                    pagedata = {
                ...
                conn.query(SourceCode_Query.List(pagedata["start"], pagedata["items"]), function (list_err, list_result) {
                    list_err ? console.log(`Getting Data Error ${list_err}`) 
                    : (()=> {
...
app.post(pageSourceCode.write_RoutePath, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            console.log("Write SourceCode Pool Connection Error : "+err);
        } else {
            conn.query(SourceCode_Query.Write(content_lang, content_tit, content_txt, cont_date, 0), function (inserterr) {
                inserterr? console.log(`Insert Contents Error ${inserterr}`) 
...
app.get(pageSourceCode.view_RoutePath, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            ...
            conn.query(SourceCode_Query.View(req.params.sCode_no), function (view_data_err, result) {
                view_data_err ? console.log(`Getting SourceCode Content Data Error ${view_data_err}`) 
                : (()=> {
                    conn.query(SourceCode_Query.ViewCnt(req.params.sCode_no), (view_cnt_err, updateResult)=>{
                        if(view_cnt_err) console.log(`Increase View Count Error ${view_cnt_err}`);
                    }); 
...
app.get(pageSourceCode.modify_RoutePath, chk_Login, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            ...
            conn.query(SourceCode_Query.View(req.params.sCode_no), function(err, result) {
...
app.post(pageSourceCode.modify_RoutePath, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            console.log("Modify SourceCode_Post Pool Connection Error : "+err);
        } else {
            conn.query(SourceCode_Query.Modify(content_lang, content_tit, content_txt, sCode_no), function (inserterr) {
                inserterr? console.log(`Insert Contents Error ${inserterr}`) 
...
app.get(pageSourceCode.delete_RoutePath, function(req, res) {
    pool.getConnection((err, conn)=> {
        if(err) {
            console.log("Delete SourceCode Pool Connection Error : "+err);
        } else {
            conn.query(SourceCode_Query.Delete(req.params.sCode_no), function(delErr) {
                delErr ? console.log('Delete Error'+delErr)
                : (()=> {
                    res.writeHead(200, {'Content-Type': 'text/html; charset=utf-8'});
                    res.write("<script>alert('삭제 완료되었습니다.')</script>");
                    res.write(`<script>location.href = '${pageSourceCode.redirect_RoutePath(10)}'</script>`);
                })();
            });
        } conn.release();
    });
...
const pageGallery = {
    ...
    delete_RoutePath : "/board/delete/gallery/:gellery_no",
...
const Gallery_Query = {
    ...
    Delete : function(gellery_no) {
        return `Delete From ${pageGallery.TB_Name} Where ${pageGallery.TB_PK} = ${gellery_no}`;
    }
}
...
app.get(pageGallery.list_RoutePath, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            ...
            conn.query(Gallery_Query.Count(), function (cnt_error, settings) {
                cnt_error ? console.log(`Setting Data Error ${cnt_error}`) 
                : (()=> {
                    pagedata = {
                ...
                conn.query(Gallery_Query.List(pagedata["start"], pagedata["items"]), function (list_err, list_result) {
                    list_err ? console.log(`Getting Data Error ${list_err}`) 
                    : (()=> {
...
app.post(pageGallery.write_RoutePath, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            console.log("Write Gallery Pool Connection Error : "+err);
        } else {
            conn.query(Gallery_Query.Write(content_user, content_tit, content_txt, cont_date, 0), function (inserterr) {
                inserterr? console.log(`Insert Contents Error ${inserterr}`) 
...
app.get(pageGallery.view_RoutePath, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            ...
            conn.query(Gallery_Query.View(req.params.gellery_no), function (view_data_err, result) {
                view_data_err ? console.log(`Getting Gallery Content Data Error ${view_data_err}`) 
                : (()=> {
                    conn.query(Gallery_Query.ViewCnt(req.params.gellery_no), (view_cnt_err, updateResult)=>{
                        if(view_cnt_err) console.log(`Increase View Count Error ${view_cnt_err}`);
                    }); 
...
app.get(pageGallery.modify_RoutePath, chk_Login, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            ...
            conn.query(Gallery_Query.View(req.params.gellery_no), function(isnerterr, result) {
...
app.post(pageGallery.modify_RoutePath, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            console.log("Modify Gallery_Post Pool Connecntion Error : "+err);
        } else {
            conn.query(Gallery_Query.Modify(content_user, content_tit, content_txt, gellery_no), function (err) {
                err? console.log(`Insert Contents Error ${err}`) 
...
app.get(pageGallery.delete_RoutePath, function(req, res) {
    pool.getConnection((err, conn)=> {
        if(err) {
            console.log("Delete Gallery Pool Connection Error : "+err);
        } else {
            conn.query(Gallery_Query.Delete(req.params.gellery_no), function(delErr) {
                delErr ? console.log('Delete Error'+delErr)
                : (()=> {
                    res.writeHead(200, {'Content-Type': 'text/html; charset=utf-8'});
                    res.write("<script>alert('삭제 완료되었습니다.')</script>");
                    res.write(`<script>location.href = '${pageGallery.redirect_RoutePath(10)}'</script>`);
                })();
            });
        } conn.release();
    });
...
const pageYoutube = {
    ...
    delete_RoutePath : "/board/delete/youTubeClip/:youtube_no",
...
const Youtube_Query = {
    ...
    Delete : function(youtube_no) {
        return `Delete From ${pageYoutube.TB_Name} Where ${pageYoutube.TB_PK} = ${youtube_no}`;
    }
}
...
app.get(pageYoutube.list_RoutePath, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            ...
            conn.query(Youtube_Query.Count(), function (cnt_error, settings) {
                cnt_error ? console.log(`Setting Data Error ${cnt_error}`) 
                : (()=> {
                    pagedata = {
                ...
                conn.query(Youtube_Query.List(pagedata["start"], pagedata["items"]), function (list_err, list_result) {
                    list_err ? console.log(`Getting Data Error ${list_err}`) 
                    : (()=> {
...
app.post(pageYoutube.write_RoutePath, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            console.log("Wirte Youtube Pool Connection Error : "+err);
        } else {
            conn.query(Youtube_Query.Write(content_user, content_tit, content_txt, cont_date, 0), function (inserterr) {
                inserterr? console.log(`Insert Contents Error ${inserterr}`) 
...
app.get(pageYoutube.view_RoutePath, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            ...
            conn.query(Youtube_Query.View(req.params.youtube_no), function (view_data_err, result) {
                view_data_err ? console.log(`Getting Gallery Content Data Error ${view_data_err}`) 
                : (()=> {
                    conn.query(Youtube_Query.ViewCnt(req.params.youtube_no), (view_cnt_err, updateResult)=>{
                        if(view_cnt_err) console.log(`Increase View Count Error ${view_cnt_err}`);
                    }); 
...
app.get(pageYoutube.modify_RoutePath, chk_Login, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            ...
            conn.query(Youtube_Query.View(req.params.youtube_no), function(err, result) {
...
app.post(pageYoutube.modify_RoutePath, function (req, res) {
    ...
    pool.getConnection((err, conn)=> {
        if(err) {
            console.log("Modify Youtube_Post Pool Connection Error : "+err);
        } else {
            conn.query(Youtube_Query.Modify(content_user, content_tit, content_txt, youtube_no), function (err) {
                err? console.log(`Insert Contents Error ${err}`) 
...
app.get(pageYoutube.delete_RoutePath, function(req, res) {
    pool.getConnection((err, conn)=> {
        if(err) {
            console.log("Delete Youtube Pool Connection Error : "+err);
        } else {
            conn.query(Youtube_Query.Delete(req.params.youtube_no), function(delErr) {
                delErr ? console.log('Delete Error'+delErr)
                : (()=> {
                    res.writeHead(200, {'Content-Type': 'text/html; charset=utf-8'});
                    res.write("<script>alert('삭제 완료되었습니다.')</script>");
                    res.write(`<script>location.href = '${pageYoutube.redirect_RoutePath(10)}'</script>`);
                })();
            });
        } conn.release();
    });
